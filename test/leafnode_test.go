...
func TestLeadNodeExportImportComplexSetup(t *testing.T) {
	...
	createAccs := func(s *server.Server) (*server.Account, nkeys.KeyPair, *server.Account, nkeys.KeyPair) {
		// Create second account with exports
		acc2, akp2 := createAccount(t, s)
		akp2Pub, _ := akp2.PublicKey()
		akp2AC := jwt.NewAccountClaims(akp2Pub)
		...
		return acc1, akp1, acc2, akp2
	}

	_, s1Akp1, _, _ := createAccs(s1)
	_, _, acc2, s2Akp2 := createAccs(s2)

	// Create the user will we use to connect the leafnode.
	kp1, _ := nkeys.CreateUser()
	pub1, _ := kp1.PublicKey()
	nuc1 := jwt.NewUserClaims(pub1)
	ujwt1, err := nuc1.Encode(s1Akp1)
	if err != nil {
		t.Fatalf("Error generating user JWT: %v", err)
	}
	...
	// Url to server s2 to which the responder will connect
	s2URL := fmt.Sprintf("nats://%s:%d", s2Opts.Host, s2Opts.Port)
	ncToS2, err := nats.Connect(s2URL, createUserCreds(t, s2, s2Akp2))
	if err != nil {
		t.Fatalf("Error on connect: %v", err)
	}
	defer ncToS2.Close()
	...
	// Pub to other account with export on original subject.
	ncToS2.Publish("foo.stream", nil)
	if _, err = lsub.NextMsg(1 * time.Second); err != nil {
		t.Fatalf("Did not receive stream message: %s", err)
	}

	// Create listener on s2
	gotIt := int32(0)
	ncToS2.Subscribe("req.echo", func(msg *nats.Msg) {
		atomic.AddInt32(&gotIt, 1)
		ncToS2.Publish(msg.Reply, []byte("WORKED"))
	})
	ncToS2.Flush()

	// TODO: need to make sure that subscription made it to LN server.
